...
Kolejny wykres przedstawia rozkład wartości LDH wśród pacjentów. 
```{r}
...
Na potrzeby eksperymentu, badania każdego pacjenta zostały pogrupowane na poziomie dnia, a wartości kolejnych atrybutów zostały uśrednione, co wpływa korzystnie na kompletność danych w każdym rekordzie. W takiej postaci znalezienie wszystkich trzech, kluczowych biomarkerów jest bardziej owocne, tzn. znaleziono 657 wyników.  Liczność badań przedstawicieli obu klas (przeżycie i śmierć) jest stosunkowo zbliżona – 341 badań pacjentów, którzy przeżyli i 316 badań pacjentów, którzy zmarli.

```{r}
biomarkers <- merged %>% select(outcome, Lactate.dehydrogenase, `High.sensitivity.C-reactive.protein`, `(%)lymphocyte`)
names(biomarkers) <- c("outcome","LDH","hs-CRP","lymphocytes")
biomarkers <- biomarkers %>% drop_na()
biomarkers
```
...
## Podział zbioru
```{r message=FALSE, warning=FALSE}
biomarkers$outcome <- factor(biomarkers$outcome)
...
confusionMatrix(data = predicted, testing$outcome)
```