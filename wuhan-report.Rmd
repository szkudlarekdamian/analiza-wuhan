...
```{r}
biomarkers <- df %>% select(Lactate.dehydrogenase, `High.sensitivity.C-reactive.protein`, `(%)lymphocyte`)
names(biomarkers) <- c("LDH","hs-CRP","lymphocytes")
biomarkers %>% drop_na()
biomarkers[!apply(biomarkers, 1, function(x) all(is.na(x))),]
